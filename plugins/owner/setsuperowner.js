// Change superOwner number — stored in Neon Postgres so it applies to ALL servers automatically
export default {
  command: 'setsuperowner',
  alias: ['changesuperowner'],
  description: 'Change superOwner number (syncs to all servers via Neon Postgres)',
  category: 'owner',
  ownerOnly: true,
  superOwnerOnly: true,
  async execute({ reply, args, db }) {
    ...
    reply(
      `✅ *SuperOwner Updated*\n\n` +
      `👑 New SuperOwner: +${num}\n` +
      `📡 Saved to Neon Postgres — applies to all servers instantly.\n\n` +
