...
const LEVELS  = ['Strangers', 'Acquaintances', 'Friends', 'Close Friends', 'Best Friends', 'Crush', 'Sweethearts 💕', 'In Love 💖', 'Soulmates 💝', 'Forever Yours 💍'];
const GIFTS   = ['🌹 Rose', '🍫 Chocolate', '🧸 Teddy Bear', '💌 Love Letter', '🎀 Gift Box', '🌸 Cherry Blossoms', '💎 Diamond', '🎵 Song'];
...
function saveGfData(senderJid, data) {
  if (!db.notes.get(GF_DB_KEY)) db.notes.set(GF_DB_KEY, '__init__', {});
  db.notes.set(GF_DB_KEY, senderJid, data);
}
...
function newGfData() {
  return {
    ...
    userName:    null,
    nickname:    null,
    anniversary: new Date().toLocaleDateString('en-US', { month: 'long', day: 'numeric', year: 'numeric' }),
    lastGift:    null,
    createdAt:   Date.now(),
  };
}
...
function rotateMood(data) {
  ...
  const now = Date.now();
  if (now - (data.moodUpdated || 0) > 30 * 60 * 1000 || data.msgCount % 10 === 0) {
...
export default {
  ...
  async execute({ sock, msg, jid, senderJid, text, react, reply, send, prefix }) {
    ...
    if (sub === 'level') {
      ...
      const nextMsg  = level < 10 ? `${10 - (data.msgCount % 10)} more messages to next level` : 'Max level reached! 💍';
      return reply(
        `💕 *Relationship Status*\n\n` +
