...
function extractFromResult(result) {
  ...
  if (!item) return null;
  // Gradio v6+ returns FileData with url + path
  ...
  // Older Gradio returns { name, data, is_file }
  if (typeof item === 'string' && item.startsWith('http')) return { url: item, name: 'video' };
  if (item?.name) return { url: item.name.startsWith('http') ? item.name : null, localPath: item.name, name: 'video' };
  return null;
}
...
async function tryZeroscope(prompt) {
  const app    = await Client.connect('hysts/zeroscope-v2', { events: [] });
...
async function tryAnimateDiff(prompt) {
  const app    = await Client.connect('ByteDance/AnimateDiff-Lightning', { events: [] });
  ...
  return { ...file, model: 'AnimateDiff-Lightning', mime: 'video/mp4', ext: 'mp4', isGif: true };
}

// ── Model 3: Wan2.1 T2V 1.3B (high quality) ──────────────────────────────────
async function tryWan21(prompt) {
  const app    = await Client.connect('Wan-AI/Wan2.1-T2V-1.3B-Diffusers', { events: [] });
...
async function tryLTX(prompt) {
  const app    = await Client.connect('Lightricks/LTX-Video', { events: [] });
  // Try to auto-detect the right endpoint
  ...
  async execute({ sock, msg, jid, text, react, reply }) {
    ...
    for (const model of MODELS) {
      try {
        ...
        // Update loading msg to show which model is being tried
        try {
          await sock.sendMessage(jid, {
            edit: loadingMsg?.key,
            text: makeLoadingText(frame, currentModel),
        ...
        // Send video
        if (fileInfo.isGif) {
          // AnimateDiff returns GIF — send as animated video
          await sock.sendMessage(jid, {
