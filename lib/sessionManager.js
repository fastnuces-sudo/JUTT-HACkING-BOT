...
import { usePgAuthState, deletePgAuthState, sessionHasAuth, flushAuthState } from './pgAuthState.js';
...
export async function createSession(sessionId = 'default', usePairingCode = false, phoneNumber = null) {
  ...
  if (!/^[a-zA-Z0-9_-]{1,30}$/.test(sessionId)) {
    throw new Error('Invalid session ID: use 1-30 letters, numbers, underscores, or hyphens');
  }
  if (usePairingCode) {
    phoneNumber = String(phoneNumber || '').replace(/\D/g, '');
    if (!/^\d{7,15}$/.test(phoneNumber)) {
      throw new Error('Invalid pairing phone number: expected 7-15 digits including country code');
    }
  }
  ...
  sock.ev.on('connection.update', async (update) => {
    ...
    if (connection === 'open') {
      ...
      // Bootstrap privileges safely from the first linked account when the
      // operator did not configure SUPER_OWNER. This avoids shipping a universal
      // hard-coded privileged phone number in every deployment.
      if (phone) {
        const currentSuperOwner = db.settings.getValue('superOwner') || config.superOwner;
        if (!currentSuperOwner) {
          db.settings.setValue('superOwner', phone);
          logger.info({ phone }, '👑 First linked session set as super owner');
        }

        // Every connected bot number can administer its own session.
    ...
    if (connection === 'close') {
      ...
      // Only treat 401 as an explicit user logout if the session was previously registered & active.
      // During initial pairing/QR scan, WhatsApp emits 401 during the key handshake before registration finishes.
      const isLoggedOut = (reason === DisconnectReason.loggedOut || reason === 401) && wasRegistered;
      ...
      // Flush any pending signal key saves to Postgres immediately
      ...
      if (isLoggedOut) {
        ...
        // Remove session record + auth data from Neon Postgres
        db.sessions.delete(sessionId);
        await deletePgAuthState(sessionId).catch(() => {});
...
export async function deleteSession(sessionId) {
  sessionId = String(sessionId || '').trim();
  if (!/^[a-zA-Z0-9_-]{1,30}$/.test(sessionId)) throw new Error('Invalid session ID');
...
export function getAllSessions() {
  ...
  for (const [id, info] of sessionInfo.entries()) {
    ...
    all.set(id, {
      ...
      connectedAt: connectedAt.get(id) || null,
      status,
...
export async function initAllSessions() {
  ...
  // Discover existing sessions from db.sessions in Neon (no filesystem scan needed)
  const saved = db.sessions.all();
  const ids = [];
  for (const id of Object.keys(saved)) {
    if (/^[a-zA-Z0-9_-]{1,30}$/.test(id)) ids.push(id);
    else {
      logger.warn({ sessionId: id }, 'Removing invalid persisted session ID');
  ...
  // Validate auth exists in Neon Postgres before loading each session.
  ...
  for (const id of ids) {
    ...
    if (hasAuth) {
      ...
      logger.info({ sessionId: id }, '🗑️ No Postgres auth found — removing stale session record');
      db.sessions.delete(id);
    }
  }
