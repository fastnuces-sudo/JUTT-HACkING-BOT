...
// Bound per-sender tracking memory on long-running public bots.
const trackerCleanup = setInterval(() => {
  const cutoff = Date.now() - 10 * 60 * 1000;
  for (const [key, lastUsed] of cooldowns) {
    if (lastUsed < cutoff) cooldowns.delete(key);
  }
  for (const [key, entry] of spamTracker) {
    if ((entry?.first || 0) < cutoff) spamTracker.delete(key);
  }
  for (const [key, entry] of _groupMetaCache) {
    if ((entry?.ts || 0) < cutoff) _groupMetaCache.delete(key);
  }
}, 10 * 60 * 1000);
trackerCleanup.unref();
...
// Marker used to detect an already-watermarked caption. Must be a substring of
// WATERMARK itself, otherwise the same caption gets stamped twice.
const WATERMARK_MARKER = 'Powered by Jutts Bot';

function withWatermark(text) {
  const value = String(text ?? '');
  return value.includes(WATERMARK_MARKER) ? value : value + WATERMARK;
}

// Captions are stamped on a copy — mutating the caller's object double-stamps
// any plugin that reuses one payload for several recipients.
function captionWithWatermark(content) {
  if (!content?.caption) return content;
  return { ...content, caption: withWatermark(content.caption) };
}
...
// Super owner is stored in Neon Postgres settings so it applies across all servers.
...
// Banned users stored in db.settings.bannedUsers (Neon Postgres) — persists & syncs across servers
...
async function getMessageText(msg) {
  ...
  // Derived from `inner`, so ephemeral-wrapped viewOnce captions resolve too.
  const voInner = inner.viewOnceMessage?.message
...
async function reply(sock, msg, text, options = {}) {
  const fullText = typeof text === 'string' ? withWatermark(text) : text;
...
async function sendMsg(sock, jid, content, options = {}) {
  ...
  if (typeof content === 'string') {
    const fullText = withWatermark(content);
    const payload = ctx ? { text: fullText, contextInfo: ctx, ...options } : { text: fullText, ...options };
    return sock.sendMessage(jid, payload);
  }
  // Non-text messages (image/audio/video/sticker): append watermark to caption if present
  const stamped = captionWithWatermark(content);
  const payload = ctx ? { contextInfo: ctx, ...stamped, ...options } : { ...stamped, ...options };
  return sock.sendMessage(jid, payload);
}
...
export async function handleMessage(sock, msg, sessionId) {
  try {
    ...
    const botMode = eff('botMode', config.botMode || 'public');
    ...
    if (eff('maintenanceMode', config.maintenanceMode) && !owner) {
    ...
    if (!owner && eff('antiSpam', config.antiSpam) && checkSpam(senderJid)) {
    ...
    if (plugin.adminOnly && isGroupMsg) {
      try {
        ...
        logger.warn({ err: error.message, jid, command }, 'Could not verify group admin permission');
        await reply(sock, msg, '⚠️ Could not verify group admin permissions. Please try again.').catch(() => {});
        return;
      }
    }
    ...
    // Remember whether "composing" was actually sent, so the matching "paused"
    // is always emitted. Re-evaluating the setting later can leave the typing
    // indicator stuck on the recipient's screen.
    const typingStarted = Boolean(eff('autoTyping', config.autoTyping)) && !fromMe && !fakeLsActive;
    if (typingStarted) {
    ...
    const _replyFn = plugin.noChannelCtx
      ? (t, opts = {}) => {
          const fullText = typeof t === 'string' ? withWatermark(t) : t;
    ...
    const _sendFn = plugin.noChannelCtx
      ? (t, opts = {}) => {
          if (typeof t === 'string') {
            return sock.sendMessage(jid, { text: withWatermark(t), ...opts });
          }
          return sock.sendMessage(jid, { ...captionWithWatermark(t), ...opts });
        }
    ...
    try {
      ...
      // Clear the indicator even when the plugin throws, otherwise the chat is
      // left showing "typing..." forever.
      if (typingStarted) {
