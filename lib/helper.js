...
export function getPrefix(text) {
  if (!text) return null;
  const dbPrefix = db.settings.getValue('prefix');
  const configured = dbPrefix ? (Array.isArray(dbPrefix) ? dbPrefix : [dbPrefix]) : (config.prefix || ['.', '!', '#', '/']);
  const prefixes = [...new Set([...configured, '.', '!', '#', '/'])];
  for (const p of prefixes) {
    if (p === '' || p === 'noprefix') return '';
...
export function parseCommand(text) {
  ...
  // Empty string is a valid "no prefix" configuration; only null means no match.
...
export function formatBytes(bytes) {
  ...
  // Clamp the index: files >= 1 TB used to index past the array and render
  // "1.00 undefined", and negative input produced "NaN undefined".
